...
def main():
	...
	# Send in the request to GET all the HIDDEN offenses, but only showing some of
	...
	# Error checking because we want to make sure the user has selected a HIDDEN offense.
	while True:
		...
		if (response.code == 200):
			...
			if (response_body['status'] != 'HIDDEN'):
				offense_ID = input('The offense you selected is not HIDDEN. Please try again or type quit. ')
			else:
