...
def main():
    ...
    # This is the array of dictionaries we're going to be using to store the data
    assignment_data = []
    ...
    if (response.code != 200):
        print('An error occured trying to get a list of offenses on the system.')
    ...
    # If there ARE offenses...
        ...
        # Create a list of just the destination networks for quick access. It's important
        # that the new list is in the same order as the original 
        ...
        for offense in offense_list:
            assigned = False    # A flag variable to show if the current offense has been
            ...
            for target_ip in offense['destination_networks']:
                ...
                if target_ip in assignment_networks:
                    ...
                    # Once we know which rule matched with the destination_network, we can
                    # send in a POST call to update the offense's assigned_to variable, and
                    # by doing that assign the offense to the correct user.
                    update_request = urllib.parse.quote('siem/offenses/' + str(offense['id']) +
                                                        '?assigned_to=' + assignment_data[index]['name'])
                    response = client.call_api(update_request, 'POST')

                    # Check the response code
                    if (response.code == 200):
                        print ('Offense ' + str(offense['id']) + ' (destination network: ' + target_ip +
                               ') has been assigned to ' + assignment_data[index]['name'] + '.')
                        assigned = True # If the offense was assigned properly, flip the flag
                    else:
                        print ('There was an error when assigning offense ' + str(offense['id']))
                        SampleUtilities.pretty_print_response(response)
                    break
            # If the offense didn't make any matches in the assignment_networks, or
            # there was a problem calling the API, display a special message.
            if (not assigned):
...
#````````````````````````````````````````````````````````````````````````````````````````#
    ...
    # Now that offenses have been assigned, we want to go through all of the offenses that
    # aren't closed and ARE assigned to people. Since we want offenses to be closed within
    # a certain time, we're going to be checking that.

    # Send in the GET request
    search = urllib.parse.quote('siem/offenses?filter=status!=CLOSED and '
                                'assigned_to is not null&fields=id,description,'
                                'start_time,destination_networks,assigned_to')
    SampleUtilities.pretty_print_request(client, search,'GET')
    response = client.call_api(search, 'GET')
    ...
    if (len(offense_list) == 0):
        ...
        # Create two new maps of assignment_data, one with the destination_networks and one
        # with the names for quick access, since both of these are necessary
        assignment_networks = list(map(lambda assignment: 
                                       assignment['destination_network'],
                                   assignment_data))
        assignment_names = list(map(lambda name: name['name'], assignment_data))
            ...
            for target_ip in offense['destination_networks']:
                ...
                if ((target_ip in assignment_networks) and 
                   (offense['assigned_to'] == assignment_names[assignment_networks.index(target_ip)])):
                    ...
                    # Get the current datetime and calculate the datetime for when the offense was
                    # created. We use these datetimes to calculate how long it's been since the offense
                    # was created.
                    # Divide start_time by 1000 since start_time is in milliseconds
                    current_date = datetime.datetime.utcnow()
                    assignment_date = datetime.datetime.utcfromtimestamp(offense['start_time']/1000)
