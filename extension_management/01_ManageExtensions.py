...
def upload_extension():
    ...
    response = client.call_api('config/extension_management/extensions', 'POST', headers = request_header, data = data)
    
    # If the response code is 201, that means the extension package has been successfully uploaded and the extension id will be returned. 
    # Otherwise -1 will be returned and the full response body is provided with error message inside.
    if (response.code != 201):
        ...
        print ('The extension has been uploaded with id = ' + str(extension_id))
        return extension_id
...
def install_extension(extension_id):
    ...
    # query parameters 
    # action_type: The desired action to take on the Extension (INSTALL or PREVIEW)
    # overwrite:  If true, any existing items on the importing system will be overwritten if the extension contains the same items. 
    #             If false, existing items will be preserved, and the corresponding items in the extension will be skipped.
    ...
    # Installing extension process is asynchronous. If 202 is returned, that means the installing task is started and the returned status id 
...
def main():
    ...
    if (extension_id != -1):
         #if extension package uploaded successfully, start installing extension task
        status_id = install_extension(extension_id)
        
        if (status_id != -1):
            #if extension installing task start wit no error, keep checking the status every 5s until the it is completed or has errors or time out.
            status = 'PROCESSING'
            
            count = 60
            while ((status == 'PROCESSING' or status == "QUEUED") and count > 0):
                status = check_install_status(status_id)
                print ('Installing status: ' + status)
                count = count - 1
                if ((status == 'PROCESSING' or status == "QUEUED") and count == 0):
                    print ('Installing process timed out.')
                    sys.exit(1)
                time.sleep(5)

            if (status == 'COMPLETED'):
                #delete the extension once it complete installed. If you want to keep the extension, please comment out the line below.
