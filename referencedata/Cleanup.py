...
def cleanup_01_sets(client):
    response = client.call_api('referencedata/sets/rest_api_samples_suspect_ips', 'DELETE')
    print(response.code)
    print(response.read().decode('utf-8'))
    response = client.call_api('referencedata/sets/rest_api_samples_blocked_ips', 'DELETE')
    print(response.code)
    print(response.read().decode('utf-8'))
...
def cleanup_02_maps(client):
    response = client.call_api('referencedata/maps/rest_api_samples_current_admin_shift', 'DELETE')
    print(response.code)
    print(response.read().decode('utf-8'))
...
def cleanup_03_map_of_sets(client):
    response = client.call_api('referencedata/mapOfSets/rest_api_samples_login_events', 'DELETE')
    print(response.code)
    print(response.read().decode('utf-8'))
...
def cleanup_04_tables(client):
    response = client.call_api('referencedata/tables/rest_api_samples_server_access', 'DELETE')
    print(response.code)
    print(response.read().decode('utf-8'))
