...
# In this scenario we have already configured two reference sets in the product,
# one to capture ip addresses exhibiting suspect behavior and another to hold
...
# with the data in our reference set so that suspected ip addresses collected by
# the product can be validated by this system and so that ip addresses blocked
# by this system can be monitored by the product to ensure they are properly
...
def main():
    ...
    SampleUtilities.pretty_print_request(client, 'referencedata/sets/rest_api_samples_suspect_ips', 'GET')
    response = client.call_api('referencedata/sets/rest_api_samples_suspect_ips', 'GET')
    ...
    for element in data:
        ...
        # For each suspect ip address, pass it to our legacy system to validate.
        # If it is a real threat, move it to the blocked list so that the
        # configured rules can generate offenses if it is active on our network.
        ip_address = element['value']
        if (legacy_system_logic(ip_address)):
            SampleUtilities.pretty_print_request(client, 'referencedata/sets/rest_api_samples_blocked_ips?value=' + ip_address, 'POST')
            response = client.call_api('referencedata/sets/rest_api_samples_blocked_ips?value=' + ip_address, 'POST')
            
            SampleUtilities.pretty_print_request(client, 'referencedata/sets/rest_api_samples_suspect_ips/' + ip_address, 'DELETE')
            response = client.call_api('referencedata/sets/rest_api_samples_suspect_ips/' + ip_address, 'DELETE')
    ...
    # us to watch.
    response = client.call_api('referencedata/sets/rest_api_samples_blocked_ips', 'GET')
    SampleUtilities.pretty_print_response(response)
