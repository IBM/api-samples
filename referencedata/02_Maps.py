...
def main():
    ...
    response = client.call_api('referencedata/maps/rest_api_samples_current_admin_shift', 'GET')
...
def setup_data(client):
    ...
    SampleUtilities.data_setup(client, 'referencedata/maps/rest_api_samples_current_admin_shift?key=7.34.10.5&value=alice', 'POST')
...
def update_reference_map(new_shift, current_shift, client):
    ...
    for server_ip in current_shift.keys():
        if  (server_ip not in new_shift.keys()):
            response = client.call_api('referencedata/maps/rest_api_samples_current_admin_shift/' + server_ip + '?value=' + current_shift[server_ip]['value'], 'DELETE')
    
    # Update the usenames to reflect the administrators on duty this shift.
    for server_ip in new_shift.keys():
        client.call_api('referencedata/maps/rest_api_samples_current_admin_shift?key='+ server_ip + '&value=' +new_shift[server_ip], 'POST')
